...
EPubDocument::EPubDocument(QObject *parent)
    : QTextDocument(parent), m_container(nullptr), m_loaded(false) {
...
QString EPubDocument::title() const
{
  return m_container->metadata("title");
}
...
{
  QStringList authors = CSVSplitter::splitOnSemicolon(m_container->metadata("authors"));
  return authors;
}
...
//  m_container->metadata("authors"] = authors;
}
